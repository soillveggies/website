...
3. **Gallery**: the gallery now pulls from a Google Sheet via a small Apps Script Web App (`gallery-apps-script.gs`, deployed from the Sheet's Extensions → Apps Script menu). Paste the deployed `/exec` URL into the `GALLERY_FEED_URL` constant near the top of `js/main.js`. Sheet columns: `title`, `category` (`indoor` / `outdoor` / `hydroponics` / `events`), `alt`, and an optional `imageUrl`. Editing the Sheet's rows updates the live site instantly — no redeploy needed; only edits to the Apps Script code itself require redeploying (Deploy → Manage deployments → Edit → New version). If `GALLERY_FEED_URL` is left blank or the feed is unreachable, the site falls back to the static placeholder data in `data/gallery.json`.
